...
################################################################################
set t epslatex size 16.4cm,3.5cm color colortext standalone header diss10pt.footnotesize
set output 'fig.tex'
...
do for [ii=0:Nfirst-1] {
  set ytics add ('$0$' ii*offset)
  set ytics add ('$\pm 20$' (ii-0.5)*offset)
}
set ytics add ('$-20$' -20)
set ytics add ('$20$' (Nfirst-1)*offset+20)
LABEL_Y = '$\hat\varepsilon(\sfpos,\sfomega)$ / dB'
# grid
load 'grid.cfg'
...
load 'xyborder.cfg'
# labels
set label 5 at graph -0.1, screen 0.94
do for [ii=1:Nfirst] {
  unset label 10+ii
}
...
plot 'array.txt' @array_active,\
  ...
  for[ii=0:Nsecond-1] 'fS.txt' every ::ii::ii using 1:2:(sprintf('$%d$',ii)) with labels offset graph 0.065,-0.035 tc ls ii+1
