...
Mlabel = '800 650 900 600 450 700 100 200 200 40'
...
#### plot 1 #####
# labels
set label 1 '\scs $\sffB_{\sfcircleh|M}$' at first 63, first 40 front left tc rgb 'red'
# x-axis
set xrange [0:100]
set xtics 10 offset 0,0.5
set xtics add ('$\infty$' 100)
...
set ytics add (40)
set logscale y
LABEL_Y = '$\sffS_{\sfcircleh|M}\;/\;\mathrm{kHz}$'
# positioning
...
plot 'Ml.txt' u 2:($1/1000) with filledcurve y1 fc rgb '#FFCCCC',\
  ...
  for[ii=1:Nspec] 'fS.txt' every ::word(Mlabel,ii)-1::word(Mlabel,ii)-1 u 1:(column(ii+1)/1000):(sprintf('$%d$', ii-1)) with labels offset graph 0.005,graph 0.075 tc ls ii,\
