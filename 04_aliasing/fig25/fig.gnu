...
do for [ii=1:words(Mvec)] {
  M = word(Mvec, ii)+0
  set cntrparam level incremental 2, 1, 2
  set table sprintf('cont_fS_M%d.txt', M)
  splot sprintf('fS_M%d_max-rE.dat', M) u 1:2:($3/1000) binary matrix w l
  unset table
  set table sprintf('cont_fM_M%d.txt', M)
  splot sprintf('fM_M%d_max-rE.dat', M) u 1:2:($3/1000) binary matrix w l
  unset table
}
